...
export async function getLatestJobStatuses(): Promise<JobRecord[]> {
  ...
  return Array.from(latestByJob.values()).sort((a, b) => {
...
export async function getDashboardStats() {
  ...
  return {
    ...
    pipelineSummary: {
      ...
      tenant_improvement: pipeline.filter(
        (j) => j.jobType === "tenant_improvement"
      ).length,
      flood_restoration: pipeline.filter(
        (j) => j.jobType === "flood_restoration"
      ).length,
